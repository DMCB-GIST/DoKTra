...
assert args.task in ['fpb','fts'] , "The task is not supported"
...
label_list_dict = {'fpb' : ["positive", "neutral", "negative"],
                   'fts' : ["positive", "neutral", "negative"]}

dataset_dict = dataset_dict_loader(args.input_dir,label_list_dict[args.task])

config_teacher = transformers.BertConfig.from_pretrained(args.config_file_teacher, num_labels=3)
config = transformers.AlbertConfig.from_pretrained(args.config_file, num_labels=3)
...
model = transformers.AlbertForSequenceClassification.from_pretrained(args.init_checkpoint,config=config)
...
tokenizer = transformers.AlbertTokenizer.from_pretrained(args.vocab_path)
...
def preprocessing_ABD(tokenizer_T,tokenizer_S, sentences,labels,datatype):
    assert datatype in ['train','dev','test'], "Wrong datatype, only train,dev,test allowed"
    input_ids_T = []
    input_ids_S = []
    for sent in sentences:
...
torch.manual_seed(args.random_seed)
expander_model = nn.Sequential(nn.Linear(768,2048))
...
def fintask_eval(prd_result,test_answer,task):
    assert task in ['fpb','fts'] , "The task is not supported"
...
if args.do_eval:
    ...
    results = fintask_eval(logits_all,[item[1] for item in dataset_dict['dev']],args.task)
    
    with open(os.path.join(args.output_dir,'eval_results.txt'),'w') as f_ev:
...
if args.do_predict:
    ...
    results = fintask_eval(logits_all,[item[1] for item in dataset_dict['test']],args.task)

    with open(os.path.join(args.output_dir,'test_eval_results.txt'),'w') as f_ev:
        f_ev.write("Accuracy: {0:.4f}\n".format(eval_accuracy/nb_eval_steps))
